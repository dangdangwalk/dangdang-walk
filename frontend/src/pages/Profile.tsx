...
// import { ResponseDogs } from '@/api/dogs';
...
function Profile() {
    ...
    return (
        <div className="mb-[60px] bg-white">
            <Topbar className="">
                <Topbar.Front className="w-12"></Topbar.Front>
                <Topbar.Center className="text-black text-lg font-bold leading-[27px]">마이페이지</Topbar.Center>
                <Topbar.Back className="" onClick={() => setIsProfileOpen(true)}>
            ...
            <main className="flex flex-col">
                <section className="flex justify-between items-center px-5 py-6">
                    <section className="flex flex-col justify-start gap-2">
                        <div className="text-neutral-800 text-lg font-bold leading-[33px]">{nickname}</div>

                        <img className="w-14" src={provider === 'kakao' ? KaKao : Google} alt="provider" />
                    </section>
            ...
            <div
                className={`fixed w-full h-full -right-full top-0 z-30 bg-slate-400 duration-200 ${isProfileOpen && '-translate-x-full'}`}
            >
                <Topbar className="">
                    <Topbar.Front className="" onClick={() => setIsProfileOpen(false)}>
                        닫기
                    </Topbar.Front>
                    <Topbar.Center className="">마이페이지</Topbar.Center>
                    <Topbar.Back
                        className=""
                        onClick={() => {
                ...
                <input type="text" value={'hello'} className="bg-white outline-none" />
            </div>
