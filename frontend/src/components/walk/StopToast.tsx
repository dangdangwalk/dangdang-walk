...
export default function StopToast({ isVisible }: { isVisible: boolean }) {
    return (
        <div
            role="alert"
            className={`w-[213px] h-14 px-[18px] bg-white rounded-2xl shadow  absolute left-1/2 -translate-x-1/2 bottom-24 z-50 
            flex justify-between items-center transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}
        >
