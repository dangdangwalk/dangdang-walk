...
TAG=$1
ENV_FILE=$2
ENV_PATH="/home/opc/actions-runner/_work/dangdang-walk/dangdang-walk/backend"
...
if [ -n "$IS_BLUE_RUNNING" ]; then
  ...
  docker pull aacho/dangdangwalk:"$TAG"
  docker run -d --name dangdang-api-green --restart always -p 3031:3031 --env-file "$FILE_PATH" -v logs:/app/log aacho/dangdangwalk:"$TAG"
  ...
  echo "Green 배포를 성공적으로 종료합니다."
   ...
   docker pull aacho/dangdangwalk:"$TAG"
   docker run -d --name dangdang-api-blue --restart always -p 3032:3031 --env-file "$FILE_PATH" -v logs:/app/log aacho/dangdangwalk:"$TAG"
